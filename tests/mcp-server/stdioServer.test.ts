...
import { BridgeHost } from "../../mcp-extension/src/bridge/bridgeHost.js";
import { TOOL_NAMES } from "../../mcp-extension/src/bridge/protocol.js";
import {
  parseServerArgs,
  type ServerCliOptions,
} from "../../mcp-server/src/server.js";

const REPOSITORY_ROOT = path.resolve(import.meta.dirname, "../..");
const BUNDLED_ENTRY = path.join(REPOSITORY_ROOT, "mcp-server", "dist", "server.cjs");
const children = new Set<ChildProcessWithoutNullStreams>();
const hosts = new Set<BridgeHost>();

beforeAll(() => {
  const built = spawnSync(process.execPath, ["esbuild.mjs"], {
    cwd: path.join(REPOSITORY_ROOT, "mcp-server"),
    encoding: "utf8",
  });
